...
public class Try44{
    ...
    public static Node buildTreeFromPreorderInorder(List<Integer> oPreOrder, List<Integer> oInorder) {
        if(oPreOrder.size() > 0 || oInorder.size() > 0) {
            ...
            parent.left = buildTreeFromPreorderInorder(preLeft, inLeft);
            parent.right = buildTreeFromPreorderInorder(preRight, inRight);
            return parent;
        }
        return null;
    }
    ...
    public  static void test3_subtract() {
        ...
        Node root = buildTreeFromPreorderInorder(preList, inList);
        Aron.inorder(root);
